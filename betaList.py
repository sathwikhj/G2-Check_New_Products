...
def scrape_betalist_topics():
    try:
        ...
        # Send a GET request to the URL
        response = requests.get('https://betalist.com/topics/software')
        ...
        if response.status_code == 200:
            ...
            # Find the main div containing all the data
            main_div = soup.find('div', class_='startups startupGrid')

            # Check if the main div is found
            if main_div:
                # Find all divs with specified classes within the main div
                divs = main_div.find_all('div', class_='startupCard')

                # Iterate over each div
                for div in divs:
                    # Find the 'a' tag containing the name
                    name_a = div.find('a', class_='block whitespace-nowrap text-ellipsis overflow-hidden font-medium')

                    # Find the 'a' tag containing the description
                    description_a = div.find('a', class_='block text-gray-500 dark:text-gray-400')

                    # Extract text from 'a' tags
                    name = name_a.text.strip() if name_a else ''
                    description = description_a.text.strip() if description_a else ''
